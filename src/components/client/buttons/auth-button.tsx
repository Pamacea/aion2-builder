...
export const AuthButton = ({ isAuthenticated, userName }: AuthButtonProps) => {
  const [isLoading, setIsLoading] = useState(false);

  const handleSignIn = () => {
    setIsLoading(true);
    // NextAuth v5: redirection vers la route de connexion Discord gérée par le catch-all [...nextauth]
    window.location.href = "/api/auth/signin/discord";
  };

  const handleSignOut = () => {
    setIsLoading(true);
    // NextAuth v5: redirection vers la route de déconnexion gérée par le catch-all [...nextauth]
  ...
  if (isAuthenticated) {
    return (
      <div className="flex items-center pr-2 gap-2 bg-background/50 border-y-2 border-foreground/50 hover:bg-background/70 hover:border-foreground/70 transition-all duration-300">
        ...
        {userName && (
          <span className="text-sm text-foreground/80 uppercase font-semibold px-2">{userName}</span>
        )}
      </div>
    );
  }
